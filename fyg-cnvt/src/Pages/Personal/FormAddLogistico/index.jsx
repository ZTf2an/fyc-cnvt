...
function FormAddLogistico ({title , submit}) {
    ...
    useEffect(()=>{
        setPoderes(valorPoderes(lI , lVi , lV , lS , logV , lB , rA));
        if ( lI ) {
            setLV(true);
            setLS(true);
            setLB(true);
        ...
        <Form id={title} onSubmit={submit}>
            ...
            <Row>
                ...
                <Col>
                    ...
                    <Form.Check type="checkbox" label="Lider Virtual" checked={lVi} onChange={e => setLVi(!lVi)}/>
                    ...
                    <Form.Check type="checkbox" label="Logistico Virtual" checked={logV} onChange={e => setLogV(!logV)}/>
                    ...
                    <Form.Control type="text" name="poderes" value={JSON.stringify(poderes)} onChange={e=>e} disabled hidden/>
                    {lI && iconoHabilidad["Lider Integral"]({size : 4})}
                    {lVi && iconoHabilidad["Lider Virtual"]({size : 4})}
                    {lV && iconoHabilidad["Lider Votación"]({size : 4})}
                    {lS && iconoHabilidad["Lider Sonido"]({size : 4})}
                    {logV && iconoHabilidad["Logistico Virtual"]({size : 4})}
                    {lB && iconoHabilidad["Logistico Basico"]({size : 4})}
                    {rA && iconoHabilidad["Redactor Actas"]({size : 4})}
